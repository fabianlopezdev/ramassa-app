...
export const SEED_ACCESS_CODE = 'blnc-k4m9-r2t7';
...
export interface PersonFixture {
  ...
  /**
   * The LOGIN IDENTIFIER, not necessarily a mailbox. Roster accounts use a
   * real-shaped `@example.test` address; the one admin-created participant
   * (RAPP-25) carries a generated `@ramassa.invalid` one, because she has no
   * inbox and that domain is reserved so it can never acquire one.
   */
  ...
  /**
   * Absent means the ordinary magic-link account. `admin_created` marks the
   * participant staff had to create by hand because she has no email: it is
   * the only kind whose password can be reset, so without one in the roster
   * the reset screen is a screen nobody can open (RAPP-25).
   */
  readonly authMethod?: 'magic_link' | 'admin_created';
  /** The typed 4-4-4 credential for the one local admin-created account. */
...
export const PARTICIPANT_FIXTURES: readonly PersonFixture[] = [
  ...
  {
    ordinal: 30,
    // Generated by `create_participant_account`, not chosen: an ASCII-folded
    // first name, a random unambiguous suffix, and a domain that can never
    // receive mail (RFC 2606). Staff hand her this string on paper.
    email: 'blnc@ramassa.invalid',
