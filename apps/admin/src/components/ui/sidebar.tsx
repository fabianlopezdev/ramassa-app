...
function useSidebar() {
  ...
  if (!context) {
    // Typed error per RAPP-12: a raw Error bypasses the logger/Sentry catalog.
    // This is a programmer error (a sidebar part rendered outside its provider),
    // so it maps to the generic unexpected code.
...
function Sidebar({
  ...
  // Translated so the mobile drawer's screen-reader header is not English-only
  // for ar/fa users (RAPP-16 a11y + RAPP-11 i18n).
      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          <SheetHeader className="sr-only">
            <SheetTitle>{t('nav:a11y.sidebarLabel')}</SheetTitle>
            <SheetDescription>{t('nav:a11y.sidebarDescription')}</SheetDescription>
          </SheetHeader>
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    ...
    >
      ...
      <span className="sr-only">{t('nav:a11y.toggleSidebar')}</span>
    </Button>
  );
}
...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  return (
    <button
      ...
      aria-label={t('nav:a11y.toggleSidebar')}
      tabIndex={-1}
      onClick={toggleSidebar}
      title={t('nav:a11y.toggleSidebar')}
      className={cn(
