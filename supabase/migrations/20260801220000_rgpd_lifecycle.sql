...
-- Telling an anonymized record apart, on the screen ----------------------------------

-- `get_participant_profile()` gains `anonymized_at`, because a screen that
-- cannot see this state offers to anonymize a record that already is, and the
-- RPC's refusal arrives as an error where a sentence would have done.
--
-- Dropped and recreated rather than `create or replace`d: adding a column to a
-- RETURNS TABLE changes the function's return type, which Postgres will not let
-- a replace do.
