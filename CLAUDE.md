...
## Workflow Contract (agreed 2026-07-16, updated 2026-08-08)
...
5. **`/react-native-perfection` runs at PHASE CLOSURE, not per task.** It is too slow per-commit. Each phase ends with a closure issue (RAPP-20, 28, 37, 40, 46, 49, 53, 56, 61, 69): full sweep -> closing commit -> direct platform QA -> QA report on the closure issue. Prefer ADB and Android platform tools on Android, stable direct simulator or XCTest tools on iOS, and Playwright on web. Maestro is a last resort only when the required behavior cannot be proved another way.
...
16. **Flow screenshots and the flow canvas are retired from the required workflow (2026-08-08, RAPP-37).** Existing captures, manifests, and canvas files are historical artifacts. No issue may require screenshots, capture freshness, a flow canvas refresh, or a standalone canvas export for acceptance. Do not run `capture:flow` or `capture:phase` unless Fabián explicitly asks for a capture. QA should produce the smallest reliable evidence for the behavior under test.
...
Flow screenshots and canvas maintenance are not closure gates. The capture harness remains in the
repository only as historical tooling and must not be run unless Fabián explicitly requests it.

1. **Android:** prefer `adb`, UI Automator, `am`, `dumpsys`, logcat, direct database assertions, and
   focused app tests. Use the smallest stable tool that proves the behavior.
2. **iOS:** prefer `simctl`, direct simulator inspection, XCTest, or XCUITest where interaction is
   required. Avoid orchestration layers when a platform tool can provide the same evidence.
3. **Web:** every new or changed web screen extends `bun run qa:web` in the same issue. The Playwright
   suite is cumulative and starts its own server on port 3100 with `--force`.
4. **Maestro:** last resort only. Use it solely when the acceptance criterion cannot be proved with
   direct platform tools, focused tests, database evidence, or Playwright. Document why it was
   necessary before running it.
